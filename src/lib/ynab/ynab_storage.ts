...
export class YnabStorage {
  ...
  readonly latestMonth: ResourceRef<Date> = resource({
    ...
    loader: async ({params}) => {
      ...
      const monthsResponse = await api.months.getBudgetMonths(
        selectedBudget.id,
      );
        ...
        response.data.accounts
          ...
          .filter(
            a =>
              ...
              a.type !== AccountType.LineOfCredit,
          )
      );
    },
  ...
  readonly status = computed<YnabStorageStatus>(() => {
    ...
    if (this.selectedBudget() === null)
      return YnabStorageStatus.NO_BUDGET_SELECTED;

    if (
      this.accounts.isLoading() ||
      this.categories.isLoading() ||
      this.latestCategories.isLoading()
    ) {
    ...
    loader: async ({params}) => {
      ...
      return (
        response.data.category_groups
          ...
          .filter(
            g => g.name.trim().toLowerCase() !== 'internal master category',
          )
      );
    },
