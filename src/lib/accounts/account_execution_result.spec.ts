...
import {
  AccountExecutionBuilder,
  AccountExecutionResult,
} from './account_execution_result';
import {
...
describe('AccountExecutionBuilder', () => {
  it('sums up values correctly', () => {
    const categories = [cat('1', 1000), cat('2', 2000)];
    const allocations = [
    ...
    const results = new AccountExecutionBuilder(
      allocations,
      categories,
    ).build();
  ...
  it('overflows values to the default account, if needed', () => {
    const categories = [cat('1', 1000), cat('2', 5000)];
    const allocations = [
      single('1', 'fake_account_1'),
      absoluteSplit(
        '2',
  ...
  it('overflows only the available amount', () => {
    const categories = [cat('1', 1000), cat('2', 500)];
    const allocations = [
      single('1', 'fake_account_1'),
      absoluteSplit(
        '2',
...
function absoluteSplit(
  categoryId: string,
  entries: AbsoluteSplitEntry[],
